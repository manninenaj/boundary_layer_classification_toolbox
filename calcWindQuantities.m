function [time_o,beta,skewn,epsilon,epsilon_error,shear_vec,shear_dir,...
    ...
    nsamples] = calcWindQuantities(data_vert,data_wind_tday,...
...
[epsilon,epsilon_error,t_epsilon,velo,velo_error,sigma_w,sigma_w_error,...
    beta,~,~,nsamples] = calcTKE(data_vert,data_wind_tday,...
...
for kk = 1:length(aatimes)
    [Xr,Yr] = meshgrid(data_wind_tday.range, aatimes{kk});
    ...
    for ir = 1:length(aatimes{kk})
        for ic = floor(win_size/2)+1:length(data_vert.range)-floor(win_size/2)
            ...
            dz = data_vert.range(ic + floor(win_size/2))-...
                data_vert.range(ic - floor(win_size/2));
...
for ifn = 1:length(fnames)
    ...
    % clean 3 lowest most range gates and skewness
    beta.(fnames{ifn})(:,1:3) = nan;
    velo.(fnames{ifn})(:,1:3) = nan;
    epsilon.(fnames{ifn})(:,1:3) = nan;
    skewn.(fnames{ifn})(:,1:3) = nan;
    sigma_w.(fnames{ifn})(:,1:3) = nan;
end
%% Time out
time_o = t_epsilon;
end
